#############################################
## Implementation of ABC to estimate the mean
## Created by Keshav Mahindra
#############################################

## Estimating the mean using Approximate Bayesian Computation (ABC)
...
for (i in 1:length(rejection.radii)) {
  ...
  for (j in 1:length(candidate.means)) {
    ...
    if (absolutediff[j] >= rejection.radii[i]) { ## Accept/Reject criteria based on absolute difference
...
plot(rejection.radii, mean.squared.error.abs)
...
## Radius is the same
## Mean seems more accurate with the first approach. The difference is negligible really.
## There is a big difference in accepted sample sizes however
